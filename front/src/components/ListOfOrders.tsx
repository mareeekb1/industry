...
const ListOfOrders:React.FC<Orders> = ({ orders }) => {
    if(orders.length === 0){
        return (
        <Box align='center'>
            <Heading level={2} color="blue">No orders found</Heading>
        </Box>
        )
    }
    const renderList = () =>{
        return orders.map((order,key)=>(
                <Box
                    ...
                    <div style={{display:'flex',width:'100%',alignItems:'center'}}>
                        ...
                        <span style={{width:'40%'}}>{dotDateFormat(order.date_of_order)}</span>
                        <span style={{width:'40%'}}>{order.sum_of_order} eur</span>
                        <span style={{width:'10%'}}>{order.number_of_products}</span>
                    </div>
                </Box>
        ))
    }
